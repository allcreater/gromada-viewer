...
public:
	...
	void drawMap(FramebufferRef framebuffer, glm::ivec2 viewportOffset, glm::ivec2 viewportSize, std::uint32_t frameCounter) {
		...
		for (const auto& [vid, spritesPack, obj] : m_visibleObjects) {
...
private:
	void updateObjectsView(glm::ivec2 min, glm::ivec2 max) {
		...
		std::ranges::sort(m_visibleObjects, {},
			[](const ObjectsView::ObjectView& obj) { return std::tuple{obj.graphics.get().visualBehavior, obj.obj.get().y + obj.graphics.get().imgHeight}; });
	}

private:
	const ObjectsView& m_objectsView;
	std::vector<ObjectsView::ObjectView> m_visibleObjects;
};